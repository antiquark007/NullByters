#!/usr/bin/env python3
# filepath: /root/NullBytes/DekstopApp/wipe-tool/windowsWipe.py
import os
import subprocess
import threading
import time
import uuid
import json
import sys
import argparse
import tempfile
from datetime import datetime

class WindowsWipeToolCLI:
    def __init__(self):
        self.stop_event = threading.Event()
        
    def run_cmd(self, command, capture_output=True):
        try:
            result = subprocess.run(command, shell=True, capture_output=capture_output, text=True)
            if result.returncode != 0:
                return None
            return result.stdout.strip()
        except Exception:
            return None

    def check_dependency(self, cmd):
        """Check if a command/tool is available on Windows"""
        return self.run_cmd(f"where {cmd}") is not None

    def list_devices(self):
        """List available storage devices on Windows"""
        try:
            devices = []
            
            # Get disk drives using wmic
            wmic_cmd = 'wmic diskdrive get DeviceID,Model,Size,SerialNumber,MediaType,InterfaceType /format:csv'
            output = subprocess.getoutput(wmic_cmd)
            
            for line in output.splitlines()[1:]:  # Skip header
                if line.strip() and ',' in line:
                    parts = [p.strip() for p in line.split(',')]
                    if len(parts) >= 6 and parts[1]:  # Check if DeviceID exists
                        device_id = parts[1]  # DeviceID (e.g., \\.\PHYSICALDRIVE0)
                        interface_type = parts[2] or "Unknown"
                        media_type = parts[3] or "Unknown" 
                        model = parts[4] or "Unknown"
                        serial = parts[5] or "Unknown"
                        size_bytes = parts[6] or "0"
                        
                        # Convert size to human readable format
                        try:
                            size_int = int(size_bytes)
                            if size_int > 0:
                                # Convert bytes to GB
                                size_gb = size_int / (1024**3)
                                if size_gb >= 1:
                                    size = f"{size_gb:.1f}G"
                                else:
                                    size_mb = size_int / (1024**2)
                                    size = f"{size_mb:.1f}M"
                            else:
                                size = "Unknown"
                        except:
                            size = "Unknown"
                        
                        # Check if removable using interface type
                        is_removable = "USB" in interface_type or "Removable" in media_type
                        
                        # Get vendor from model
                        vendor = ""
                        if model and model != "Unknown":
                            vendor_names = ["WDC", "Seagate", "Samsung", "Toshiba", "Hitachi", "SanDisk", "Kingston", "Crucial", "Intel", "Corsair"]
                            for v in vendor_names:
                                if v.lower() in model.lower():
                                    vendor = v
                                    break
                        
                        # Determine device type
                        device_type = "Unknown"
                        if "SSD" in model.upper():
                            device_type = "SSD"
                        elif "USB" in interface_type:
                            device_type = "USB Drive"
                        elif "IDE" in interface_type:
                            device_type = "IDE Drive"
                        elif "SATA" in interface_type:
                            device_type = "SATA Drive"
                        elif "SCSI" in interface_type:
                            device_type = "SCSI Drive"
                        elif "NVMe" in model.upper():
                            device_type = "NVMe SSD"
                        
                        device_info = {
                            "name": f"{vendor} {model}".strip() if vendor else model,
                            "path": device_id,
                            "size": size,
                            "model": model,
                            "serial": serial,
                            "removable": is_removable,
                            "vendor": vendor,
                            "interface": interface_type,
                            "media_type": media_type,
                            "device_type": device_type
                        }
                        devices.append(device_info)
            
            return devices
                
        except Exception as e:
            return []

    def print_devices_json(self, devices):
        """Print devices in JSON format for GUI consumption"""
        output = {
            "devices": devices,
            "count": len(devices),
            "timestamp": datetime.now().isoformat(),
            "platform": "Windows"
        }
        print(json.dumps(output, indent=2))
        sys.stdout.flush()

    def is_device_mounted(self, device):
        """Check if device is mounted on Windows"""
        try:
            # Extract drive number from device path (e.g., \\.\PHYSICALDRIVE0 -> 0)
            if 'PHYSICALDRIVE' in device:
                drive_num = device.split('PHYSICALDRIVE')[-1]
                # Check for associated volumes
                volumes_cmd = f'wmic volume where "DriveLetter IS NOT NULL" get DriveLetter /format:csv'
                output = subprocess.getoutput(volumes_cmd)
                # If there are volumes, the drive might be in use
                return len([line for line in output.splitlines() if line.strip() and ':' in line]) > 0
            return False
        except:
            return False

    def is_system_drive(self, device):
        """Check if device is a system drive on Windows"""
        try:
            # Get system drive letter
            system_drive = os.environ.get('SystemDrive', 'C:')
            
            # Usually PHYSICALDRIVE0 contains the system partition on Windows
            if 'PHYSICALDRIVE0' in device:
                return True
                
            # Additional check: see if the device contains the Windows directory
            system_root = os.environ.get('SystemRoot', 'C:\\Windows')
            if system_root[0].upper() + ':' in device:
                return True
                
            return False
        except:
            return False

    def get_wipe_command(self, device, method):
        """Get the appropriate wipe command based on method for Windows"""
        # PowerShell-based commands for Windows
        if method == "clear":
            # Single pass zero fill
            cmd = f'''powershell -Command "& {{
                try {{
                    $device = '{device}'
                    Write-Host 'Opening device handle...'
                    $fileStream = [System.IO.File]::OpenWrite($device)
                    $buffer = New-Object byte[] 1048576  # 1MB buffer
                    $totalSize = $fileStream.Length
                    $written = 0
                    
                    while ($written -lt $totalSize) {{
                        $toWrite = [Math]::Min(1048576, $totalSize - $written)
                        $fileStream.Write($buffer, 0, $toWrite)
                        $written += $toWrite
                        $percent = [Math]::Round(($written / $totalSize) * 100, 2)
                        Write-Host "Progress: $percent% ($written / $totalSize bytes)"
                        Start-Sleep -Milliseconds 10
                    }}
                    
                    $fileStream.Close()
                    Write-Host 'Clear operation completed successfully'
                }} catch {{
                    Write-Error "Error: $($_.Exception.Message)"
                    exit 1
                }}
            }}"'''
        
        elif method == "purge":
            # 3-pass wipe with different patterns
            cmd = f'''powershell -Command "& {{
                try {{
                    $device = '{device}'
                    $patterns = @([byte]0x00, [byte]0xFF, [byte]0x55)
                    
                    foreach ($i in 0..2) {{
                        Write-Host "Starting pass $($i + 1) of 3..."
                        $fileStream = [System.IO.File]::OpenWrite($device)
                        $buffer = New-Object byte[] 1048576
                        for ($j = 0; $j -lt $buffer.Length; $j++) {{ $buffer[$j] = $patterns[$i] }}
                        
                        $totalSize = $fileStream.Length
                        $written = 0
                        
                        while ($written -lt $totalSize) {{
                            $toWrite = [Math]::Min(1048576, $totalSize - $written)
                            $fileStream.Write($buffer, 0, $toWrite)
                            $written += $toWrite
                            $percent = [Math]::Round(($written / $totalSize) * 100, 2)
                            Write-Host "Pass $($i + 1) Progress: $percent%"
                            Start-Sleep -Milliseconds 10
                        }}
                        
                        $fileStream.Close()
                        Write-Host "Pass $($i + 1) completed"
                    }}
                    
                    Write-Host 'Purge operation completed successfully'
                }} catch {{
                    Write-Error "Error: $($_.Exception.Message)"
                    exit 1
                }}
            }}"'''
        
        else:  # destroy - 7 pass DoD wipe
            cmd = f'''powershell -Command "& {{
                try {{
                    $device = '{device}'
                    $patterns = @([byte]0x00, [byte]0xFF, [byte]0x55, [byte]0xAA, [byte]0x33, [byte]0xCC, [byte]0x00)
                    
                    foreach ($i in 0..6) {{
                        Write-Host "Starting DoD pass $($i + 1) of 7..."
                        $fileStream = [System.IO.File]::OpenWrite($device)
                        $buffer = New-Object byte[] 1048576
                        for ($j = 0; $j -lt $buffer.Length; $j++) {{ $buffer[$j] = $patterns[$i] }}
                        
                        $totalSize = $fileStream.Length
                        $written = 0
                        
                        while ($written -lt $totalSize) {{
                            $toWrite = [Math]::Min(1048576, $totalSize - $written)
                            $fileStream.Write($buffer, 0, $toWrite)
                            $written += $toWrite
                            $percent = [Math]::Round(($written / $totalSize) * 100, 2)
                            Write-Host "DoD Pass $($i + 1) Progress: $percent%"
                            Start-Sleep -Milliseconds 10
                        }}
                        
                        $fileStream.Close()
                        Write-Host "DoD Pass $($i + 1) completed"
                    }}
                    
                    Write-Host 'Destroy operation completed successfully'
                }} catch {{
                    Write-Error "Error: $($_.Exception.Message)"
                    exit 1
                }}
            }}"'''
        
        return cmd

    def print_progress(self, percent, message=""):
        """Print progress in JSON format for GUI"""
        progress_data = {
            "progress": percent,
            "message": message,
            "timestamp": datetime.now().isoformat(),
            "platform": "Windows"
        }
        print(json.dumps(progress_data))
        sys.stdout.flush()

    def run_wipe_command(self, cmd, log_path):
        """Run wipe command with progress monitoring on Windows"""
        try:
            with open(log_path, "w", encoding='utf-8') as logfile:
                # Start the process
                proc = subprocess.Popen(
                    cmd, 
                    shell=True, 
                    stdout=subprocess.PIPE, 
                    stderr=subprocess.STDOUT,
                    universal_newlines=True,
                    bufsize=1
                )
                
                progress = 0
                while proc.poll() is None:
                    if self.stop_event.is_set():
                        proc.terminate()
                        return False
                        
                    # Read output
                    line = proc.stdout.readline()
                    if line:
                        logfile.write(line)
                        logfile.flush()
                        
                        # Parse Windows PowerShell progress output
                        if "Progress:" in line or "Pass" in line:
                            try:
                                if "%" in line:
                                    # Extract percentage from line
                                    percent_str = line.split("%")[0].split()[-1]
                                    progress = float(percent_str)
                                    self.print_progress(int(progress), f"Windows wipe: {line.strip()}")
                                else:
                                    progress = min(progress + 2, 99)
                                    self.print_progress(int(progress), f"Windows wipe: {line.strip()}")
                            except:
                                progress = min(progress + 1, 99)
                                self.print_progress(int(progress), "Windows wipe in progress...")
                    
                    time.sleep(0.1)
                
                # Process completed
                return_code = proc.wait()
                if return_code == 0:
                    self.print_progress(100, "Windows wipe completed successfully")
                    return True
                else:
                    self.print_progress(0, f"Windows wipe failed with code {return_code}")
                    return False
                    
        except Exception as e:
            self.print_progress(0, f"Error during Windows wipe: {str(e)}")
            return False

    def verify_wipe(self, device):
        """Verify if device was properly wiped on Windows"""
        try:
            verify_cmd = f'''powershell -Command "& {{
                $fs = New-Object System.IO.FileStream('{device}', [System.IO.FileMode]::Open, [System.IO.FileAccess]::Read)
                $buffer = New-Object byte[] 512
                $fs.Read($buffer, 0, 512)
                $fs.Close()
                $sum = ($buffer | Measure-Object -Sum).Sum
                Write-Output $sum
            }}"'''
            result = subprocess.getoutput(verify_cmd)
            return int(result.strip()) == 0
        except Exception:
            return False

    def check_admin_privileges(self):
        """Check if running with administrator privileges"""
        try:
            import ctypes
            return ctypes.windll.shell32.IsUserAnAdmin()
        except:
            return False

    def get_windows_version(self):
        """Get Windows version information"""
        try:
            import platform
            return f"{platform.system()} {platform.release()}"
        except:
            return "Windows Unknown"

    def create_wipe_log(self, device, method, log_file, status, verified_clean, output_file):
        """Create comprehensive wipe log for Windows"""
        # Get device information
        device_info = next((d for d in self.list_devices() if d["path"] == device), {})
        
        # Get appropriate temp directory and user
        temp_dir = os.environ.get('TEMP', 'C:\\temp')
        operator = os.environ.get('USERNAME', 'Unknown')
        
        wipe_log = {
            "version": "1.0",
            "device": {
                "path": device,
                "name": device_info.get("name", "Unknown Device"),
                "model": device_info.get("model", "Unknown"),
                "serial": device_info.get("serial", "Unknown"),
                "size": device_info.get("size", "Unknown"),
                "vendor": device_info.get("vendor", "Unknown"),
                "interface": device_info.get("interface", "Unknown"),
                "device_type": device_info.get("device_type", "Unknown")
            },
            "wipe": {
                "method": method,
                "nist_level": self.get_nist_level(method),
                "status": status,
                "started_at": datetime.now().isoformat(),
                "finished_at": datetime.now().isoformat(),
                "passes_completed": self.get_pass_count(method),
                "verified_clean": verified_clean,
                "tools_used": ["PowerShell", "FileStream API"]
            },
            "system": {
                "tool_version": "1.0.0-windows",
                "platform": "Windows",
                "os_name": os.name,
                "operator": operator,
                "log_file": log_file,
                "windows_version": self.get_windows_version(),
                "admin_privileges": self.check_admin_privileges()
            },
            "compliance": {
                "nist_800_88": True,
                "certificate_id": str(uuid.uuid4()),
                "dod_5220_22_m": method in ["purge", "destroy"]
            }
        }
        
        try:
            # Ensure output directory exists
            os.makedirs(os.path.dirname(output_file), exist_ok=True)
            
            with open(output_file, 'w') as f:
                json.dump(wipe_log, f, indent=2)
        except Exception as e:
            # Fallback to temp directory
            temp_file = os.path.join(temp_dir, f"wipe_log_{int(time.time())}.json")
            with open(temp_file, 'w') as f:
                json.dump(wipe_log, f, indent=2)
            wipe_log["system"]["log_file"] = temp_file
        
        return wipe_log

    def get_nist_level(self, method):
        """Get NIST compliance level for method"""
        levels = {
            "clear": "clear",
            "purge": "purge", 
            "destroy": "destroy"
        }
        return levels.get(method, "clear")

    def get_pass_count(self, method):
        """Get number of passes for method"""
        passes = {
            "clear": 1,
            "purge": 3,
            "destroy": 7
        }
        return passes.get(method, 1)

    def wipe_device(self, device_path, method, output_log):
        """Main wipe function for Windows"""
        # Check administrator privileges
        if not self.check_admin_privileges():
            print(json.dumps({"error": "Administrator privileges required", "code": 3}))
            return False
            
        # Safety checks
        if self.is_system_drive(device_path):
            print(json.dumps({"error": "Cannot wipe system drive", "code": 1}))
            return False
            
        if self.is_device_mounted(device_path):
            print(json.dumps({"error": "Device appears to be in use", "code": 2}))
            return False
        
        # Start wipe process
        self.print_progress(0, "Starting Windows wipe process...")
        
        # Get wipe command
        cmd = self.get_wipe_command(device_path, method)
        
        # Create log file path
        temp_dir = os.environ.get('TEMP', 'C:\\temp')
        log_file = os.path.join(temp_dir, f"wipe_raw_{int(time.time())}.log")
        
        # Run wipe
        success = self.run_wipe_command(cmd, log_file)
        
        # Verify wipe if successful
        verified_clean = False
        if success:
            self.print_progress(95, "Verifying Windows wipe...")
            verified_clean = self.verify_wipe(device_path)
        
        # Create comprehensive log
        status = "success" if success else "failed"
        if not output_log:
            output_log = os.path.join(temp_dir, f"wipe_log_{int(time.time())}.json")
            
        wipe_log = self.create_wipe_log(
            device_path, method, log_file, status, verified_clean, output_log
        )
        
        self.print_progress(100, "Windows wipe process completed")
        
        return success

def main():
    parser = argparse.ArgumentParser(description='NullBytes Secure Wipe Tool - Windows')
    parser.add_argument('--list', action='store_true', help='List available devices')
    parser.add_argument('--json', action='store_true', help='Output in JSON format')
    parser.add_argument('--device', help='Device path to wipe')
    parser.add_argument('--method', choices=['clear', 'purge', 'destroy'], 
                       default='clear', help='Wipe method')
    parser.add_argument('--output', help='Output log file path')
    parser.add_argument('--check-admin', action='store_true', help='Check if running with admin privileges')
    
    args = parser.parse_args()
    
    wipe_tool = WindowsWipeToolCLI()
    
    if args.check_admin:
        is_admin = wipe_tool.check_admin_privileges()
        print(json.dumps({"admin": is_admin, "platform": "Windows"}))
        return
    
    if args.list:
        devices = wipe_tool.list_devices()
        if args.json:
            wipe_tool.print_devices_json(devices)
        else:
            for device in devices:
                print(f"{device['path']} - {device['name']} ({device['size']}) - {device['device_type']}")
    
    elif args.device:
        if not args.output:
            temp_dir = os.environ.get('TEMP', 'C:\\temp')
            args.output = os.path.join(temp_dir, f"wipe_log_{int(time.time())}.json")
        
        success = wipe_tool.wipe_device(args.device, args.method, args.output)
        sys.exit(0 if success else 1)
    
    else:
        parser.print_help()

if __name__ == "__main__":
    main()